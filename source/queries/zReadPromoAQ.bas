...
Where ="(((zPromo.EffDate)>=ZEDtEff()) AND ((zPromo.ExpDate)<=ZEDtExp()) AND ((zPromo.Ap"
    ...
    "ftPromo)=False) AND ((zPromo.Typ1)=True)) OR (((zPromo.Approvd)=True) AND ((zPro"
    "mo.Deletd)=False) AND ((zPromo.SWO)=False) AND ((zPromo.DftPromo)=False) AND ((z"
    "Promo.RedeemDt)>=Now()) AND ((zPromo.Typ1)=True))"
Begin InputTables
    Name ="zPromo"
End
Begin OutputColumns
