            ...
            Begin
                ...
                Begin UnboundObjectFrame
                    ...
                    OleData = Begin
                        ...
                        0x0000004600000000000000000000000080bb9a77e907d30103000000800c0000 ,
                ...
                Begin ComboBox
                    ...
                    RowSource ="SELECT DISTINCT zVendor.Company, IIf(IsNull([CSubVCs]),\"Must Assign Sub Vendor "
                        "Codes\",[CSubVCs]) AS SubCds FROM (zSubVCQ RIGHT JOIN zVendor ON zSubVCQ.VendorC"
                        "ode = zVendor.VendorCode) INNER JOIN zPromo ON zVendor.ID = zPromo.ID WHERE (((z"
                        "Promo.DftPromo)=-1) AND ((zPromo.EffYr)=ZEffYr() Or (zPromo.EffYr)=ZEffYr()+1 Or"
                        " (zPromo.EffYr)=ZEffYr()-1)) OR (((zPromo.DftPromo)=-1) AND ((zPromo.EffDate)<No"
                        "w()) AND ((zPromo.RedeemDt)>Now())) OR (((zPromo.DftPromo)=-1) AND ((zPromo.EffY"
                        "r) Is Null) AND ((zPromo.EffDate) Is Null) AND ((zPromo.RedeemDt) Is Null)) ORDE"
                        "R BY zVendor.Company;"
                ...
                Begin Subform
                    ...
                    TabIndex =16
                    Name ="zVndDefProSF"
                    SourceObject ="Form.zVndDefProSF"
                    LinkChildFields ="ID"
                    LinkMasterFields ="ID"
...
Private Sub SelGoVndr_AfterUpdate()
   ...
   Dim rst As Recordset, strCriteria As String
    ...
    Z_ID = Me!ID ' Get the Marketing ID
...
On Error GoTo Err_GoRecID_Click
    ...
    Else
        Me.Bookmark = rst.Bookmark
        'DoCmd.GoToControl ""
        'SetFocus.zVndDefProSF  'zPromoSubSelRecF"
        'Me.zVndDefProSF.SetFocus  '4/17/15
        'DoCmd.GoToControl "
        'Me.RecID.SetFocus  '4/17/15
        
        DoCmd.FindRecord intRec, acEntire, , acSearchAll, , acCurrent
        'DoCmd.GoToControl "
        'Me.Unbnd.SetFocus  '4/17/15
    End If
...
If IsNull(Me!ID) Or Me!ID = "" Then
  Me!ID = ZID() 'Me!zVndDefProF.ID
End If

Exit_zVndDefProSF_Enter:
    Exit Sub

Err_zVndDefProSF_Enter:
    MsgBox Err.Description
    Resume Exit_zVndDefProSF_Enter
End Sub
