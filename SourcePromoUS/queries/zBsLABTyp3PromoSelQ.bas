...
Where ="(((zPromo.EffDate)>=ZEDtEff()) AND ((zPromo.ExpDate)<=ZEDtExp()) AND ((zPromo.Ap"
    ...
    ")=ZEffYr() Or (zPromo.EffYr)=ZEffYr()-1)) OR (((zPromo.EffDate)>=ZEDtEff()) AND "
    "((zPromo.ExpDate)<=ZEDtExp()) AND ((zPromo.Approvd)=True) AND ((zPromo.Deletd)=F"
    "alse) AND ((zPromo.SWO)=False) AND ((zPromo.DftPromo)=False) AND ((zPromo.Typ3)="
    "True) AND ((zPromo.RedeemDt) Is Not Null)) OR (((zPromo.Approvd)=True) AND ((zPr"
    "omo.Deletd)=False) AND ((zPromo.SWO)=False) AND ((zPromo.DftPromo)=False) AND (("
    "zPromo.Typ3)=True) AND ((zPromo.RedeemDt)>=Now())) OR (((zPromo.Approvd)=True) A"
    "ND ((zPromo.Deletd)=False) AND ((zPromo.SWO)=False) AND ((zPromo.DftPromo)=False"
    ") AND ((zPromo.Typ3)=True) AND ((zPromo.EffYr)=ZEffYr() Or (zPromo.EffYr)=ZEffYr"
    "()-1))"
Begin InputTables
    Name ="zPromo"
...
Begin OutputColumns
    ...
    Alias ="Div"
    Expression ="IIf([Typ1]=-1,\"Dental\",IIf([Typ3]=-1,\"LAB\",\"Dental\"))"
    Alias ="RdmDt"
...
dbText "Description" ="[Typ3]=\"Zahn National Promotions\" One2one!!!"
